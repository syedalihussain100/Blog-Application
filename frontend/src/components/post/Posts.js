...
export default function PostsList() {
  ...
  const { PostList, appErr, serverErr, likes, disLikes } = storeData;
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-wrap -mx-3">
              <div className="mb-8 lg:mb-0 w-full lg:w-1/4 px-3">
                <div className="py-4 px-6 bg-gray-600 shadow rounded">
                  ...
                  <ul>
                    {CateLoading ? (
                      <LoadingComponent />
                    ) : CateAppErr || CateServerErr ? (
                      <h1>
                        {CateServerErr.message} {CateAppErr}
                      </h1>
                      ...
                      categoryList?.map((category) => (
                        <li key={category?._id}>
                          <p
                            onClick={() =>
                              dispatch(PostsListAction(category?.title))
                            }
              ...
              <div className="w-full lg:w-3/4 px-3">
                {appErr || serverErr ? (
                  <h1 className="text-red-500 text-center text-lg">{appErr}</h1>
                ) : PostList?.length <= 0 ? (
                  <h1 className="text-red-500">No Post Found</h1>
                ) : (
                  PostList?.map((post) => (
                    <div
                      key={post?._id}
