...
import { sendMailAction } from "../../../redux/Slices/SentEmail/emailSlice";
...
const SendEmail = () => {
  ...
  const MailData = useSelector((state) => state.sentMail);

  const { profile } = storeData;
  const { appErr, loading, serverErr, isMailSent } = MailData;
  //dispath
  ...
  if (isMailSent) return navigate(`/profile/${profile?._id}`);
  return (
